...
class JQFile
{
    ...
    public function __construct($options = null, $initialize = true, $error_messages = null) {
        if (! file_exists(public_path() . '/files/resized')) {
            File::makeDirectory(public_path() . '/files/resized', $mode = 0777, true, true);
        }

        if (! file_exists(public_path() . '/temp/thumbnail')) {
            File::makeDirectory(public_path() . '/temp/thumbnail', $mode = 0777, true, true);
        }

        $this->options = array(
            'script_url' => public_path() . '/temp/',
            'upload_dir' => public_path() . '/temp/',
            'upload_url' => '/temp/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'selectedFilesForUpload',
            ...
            'image_file_types' => '/\.(gif|jpe?g|png|svg|ico|bmp|tif|tiff)$/i',
    ...
    function get_config_bytes($val) {
        ...
        $val = (int) (is_numeric($last)) ? $val : substr(trim($val), 0, -1);
    ...
    protected function trim_file_name($name,
        ...
        if (strpos($name, '.') === false &&
            preg_match('/^image\/(gif|jpe?g|png|svg|ico|bmp|tif|tiff)/', $type, $matches)) {
            $name .= '.'.$matches[1];
        }
        return $name;
    }
